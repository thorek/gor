...
var entity_config_builder_1 = require("../builder/entity-config-builder");
var enum_config_builder_1 = require("../builder/enum-config-builder");
/**
 *
 */
var Gor = /** @class */ (function () {
     ...
     *
     */
    Gor.prototype.addCustomEntities = function () {
        var _a;
        var types = [];
        for (var _i = 0; _i < arguments.length; _i++) {
            types[_i] = arguments[_i];
        }
        (_a = this.customEntities).push.apply(_a, types);
    };
    /**
     *
     */
    Gor.prototype.schema = function () {
        return __awaiter(this, void 0, void 0, function () {
            ...
            return __generator(this, function (_a) {
                ...
                configEntities = this.getConfigTypes();
                defaultFilterTypes = this.getScalarFilterTypes();
                types = __spreadArrays(defaultFilterTypes, this.customEntities, configEntities);
     ...
     *
     */
    Gor.prototype.getConfigTypes = function () {
        var _this = this;
        return lodash_1.default.flatten(lodash_1.default.map(this.configs, function (resolver, folder) {
            ...
            return lodash_1.default.compact(lodash_1.default.flatten(lodash_1.default.map(files, function (file) { return _this.createConfigurationTypes(folder, file, resolver); })));
        }));
    };
    /**
     *
     */
    Gor.prototype.getScalarFilterTypes = function () {
        return lodash_1.default.flatten(lodash_1.default.map(this.configs, function (resolver, folder) {
            if (!resolver)
                resolver = new no_resolver_1.NoResolver();
     ...
     *
     */
    Gor.prototype.createConfigurationTypes = function (folder, file, resolver) {
        var builder = [];
        try {
            file = path_1.default.join(folder, file);
            var content = fs_1.default.readFileSync(file).toString();
            var configs = yaml_1.default.parse(content);
            builder.push.apply(builder, lodash_1.default.map(configs['entity'], function (config, name) { return entity_config_builder_1.EntityConfigBuilder.create(name, resolver, config); }));
            builder.push.apply(builder, lodash_1.default.map(configs['enum'], function (config, name) { return enum_config_builder_1.EnumConfigBuilder.create(name, resolver, config); }));
        }
        catch (e) {
            console.warn("[" + file + "]: " + e);
        }
